...
class TodoListViewModel(
    ...
    val taskItemsLiveData: LiveData<List<TaskListItem>> = MediatorLiveData<List<TaskListItem>>().apply {
        addSource(queryLiveData) { value = calculateList(queryLiveData.value, _taskItemsLiveData.value!!) }
        addSource(_taskItemsLiveData) { value = calculateList(queryLiveData.value, _taskItemsLiveData.value!!) }
    }
    ...
    private fun calculateList(query: String?, originalList: List<TaskListItem>): List<TaskListItem> {
